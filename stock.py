...
def kerr_update2(db):  # 找出對應號碼, update股價
    ...
    for num in range(len(stock_Name)):
        ...
        WHERE STOCK_NUM = %s" % (stock_Name[num][3], stock_Name[num][4], stock_Name[num][5], stock_Name[num][6],
                                 stock_Name[num][7], stock_Name[num][8], stock_Name[num][9], stock_Name[num][10],
                                 stock_Name[num][11],
                                 stock_Name[num][1])
        print(sql)
        cur.execute(sql)
...
def df_test2(number):
    ...
    mpf.volume_overlay(ax2, df_2330['Open'], df_2330['Close'], df_2330['Volume'], colorup='r', colordown='g', width=0.5,
                       alpha=0.8)
...
def schedule():
    ...
    scheduler.add_job(soup_stock, 'interval', days=1, start_date=time.strftime('%Y-%m-05 00:00:00'),
                      end_date=time.strftime('%Y-%m-15 00:00:00'))
    scheduler.start()
