...
class OracleConnection:
    ...
    def _get_oracle_conn(self):  # 連結數據庫 env 0: dev02 , 1:188
        ...
        oracle_ = {'password': ['LF64qad32gfecxPOJ603', 'JKoijh785gfrqaX67854', 'eMxX8B#wktFZ8V'],
                   ...
                   'sid': ['firefog', 'game', '']}
        self._conn = cx_Oracle.connect(username, oracle_['password'][self._env_id],
                                       oracle_['ip'][self._env_id] + ':1521/' +
                                       oracle_['sid'][self._env_id] + service_name)
        return self._conn
    ...
    def select_domain_default_url(self, domain):
        with self._get_oracle_conn().cursor() as cursor:
    ...
    def select_user_id(self, account_, joint_type=None):
        with self._get_oracle_conn().cursor() as cursor:
...
class MysqlConnection:
    ...
    def get_mysql_conn(self, third):  # 第三方  mysql連線
        if self._conn is not None:
            return self._conn
    ...
    def thirdly_tran(self, tran_type, third, user) -> list:
