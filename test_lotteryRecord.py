...
def select_numberRecord(conn,lottery,num):#開獎號
    with conn.cursor() as cursor:
        sql = "select * from (select number_record , rank() over (partition by lotteryid order by sale_start_time desc) as rank_num from game_issue where lotteryid = %s and number_record is not NULL and  sale_start_time <  sysdate and  sale_start_time >  sysdate - 20) a where rank_num <=  %s"%(lottery_dict[lottery][1],num)
...
def record_other(lottery,envs,mulissue):#戰報 做 百十位 的計算, big 判斷彩種大小連界參數
    ...
    while True:
        for number_ in digit_dict:#百,十,個
            ...
            msg3.append("%s 彩種近%s期開獎號: %s"%(lottery_dict[lottery][0],mulissue,number_record)+'%s 開獎號: %s'%(number_,digit_dict[number_]))
...
def record_dice(lottery,envs,mulissue):#  戰報 完法 取總和 
    ...
    while True:
        ...
        msg3.append("%s 彩種近%s期開獎號: %s"%(lottery_dict[lottery][0],mulissue,number_record)+"開獎號總和: %s"%(record_total))
...
def all_lottery(envs,mulissue=3):#戰報顯示, envs環境 0:dev ,1:188, mulissue: 抓取幾期
    ...
    'cqssc','btcffc','txffc','llssc','jlffc','fhjlssc','hljssc','xjssc','shssl','tjssc']#顯示彩種順序
